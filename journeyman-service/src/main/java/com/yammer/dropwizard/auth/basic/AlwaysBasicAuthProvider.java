 ...
 */
public class AlwaysBasicAuthProvider<T> implements InjectableProvider<HttpMethod, Type> {
    private static final Logger LOGGER = LoggerFactory.getLogger(AlwaysBasicAuthProvider.class);
                    ...
                    if (space > 0) {
                        ...
                        if (PREFIX.equalsIgnoreCase(method)) {
                            ...
                            if (i > 0) {
                                ...
                                if (result.isPresent()) {
                                    c.getProperties().put("AUTH", result.get());
                                    return result.get();
                                }
                            }
                        }
     ...
     */
    public AlwaysBasicAuthProvider(Authenticator<BasicCredentials, T> authenticator, String realm) {
        this.authenticator = authenticator;
        this.realm = realm;
    }
    ...
    public Injectable<?> getInjectable(ComponentContext ic,
                                       HttpMethod a,
                                       Type c) {
        return new BasicAuthInjectable<T>(authenticator, realm, true);
    }
}
